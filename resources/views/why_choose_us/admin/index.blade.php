    ...
    @push('scripts')
        <script>
            let dataGrid = $('#dg').datagrid({
                title: '&nbsp;لیست کاربران',
                url: '{{ route('admin.why_choose_us.items') }}',
                columns: [[
                    ...
                    {field: 'id', sortable: true, title: 'شناسه', align: 'center'},
                    {
                        ...
                        formatter: function (val, row) {
                            return '<a href="' + '{{ route('admin.why_choose_us.index') }}' + '/' + row.id + '" target="_blank">' + val + '</a>';
                        }
                    },
                ...
                toolbar: [
                    {
                        ...
                        iconCls: 'fa fa-eye',
                        handler: function () {
                            window.open('{{ route('admin.why_choose_us.index') }}' + '/' + id(), '_blank');
                        }
                    }, '-', {
                        text: 'ویرایش',
                        iconCls: 'fa fa-pencil',
                        handler: function () {
                            window.open('{{ route('admin.why_choose_us.index') }}' + '/' + 'edit' + '/' + id(), '_blank');
                        }
                    }, '-', {
                        text: 'حذف',
                            ...
                            $.confirm({
                                ...
                                buttons: {
                                    ...
                                    confirm: {
                                        ...
                                        action: function () {
                                            $.post('{{ route('admin.why_choose_us.index') }}' + '/' + ids(), {_method: 'delete'}).done(function (response) {
                                                if (response['error'])
                                                    $.alert({
                                                        title: 'خطا!',
